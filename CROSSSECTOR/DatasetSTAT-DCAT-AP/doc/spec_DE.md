...
- `accessRights`: Diese Eigenschaft bezieht sich auf Informationen, die angeben, ob das Dataset offene Daten sind, Zugangsbeschränkungen haben oder nicht öffentlich sind. Ein kontrolliertes Vokabular mit drei Mitgliedern (:public, :restricted, :non-public) wird vom Amt für Veröffentlichungen der EU erstellt und gepflegt. Enum:'public, restricted, non-public'  - `alternateName`: Ein alternativer Name für diesen Artikel  - `contactPoint`: Sie entspricht der obligatorischen Eigenschaft "contact point" von DCAT-AP 2.0.1. Diese Eigenschaft enthält Kontaktinformationen, die zum Senden von Kommentaren zum Dataset verwendet werden können.  - `creator`: Diese Eigenschaft bezieht sich auf die Entität, die hauptsächlich für die Erstellung des Katalogs verantwortlich ist  - `dataProvider`: Eine Folge von Zeichen, die den Anbieter der harmonisierten Dateneinheit identifiziert.  - `datasetDescription`: Diese Eigenschaft enthält eine Freitextbeschreibung des Datasets. Sie entspricht der obligatorischen Eigenschaft "description" von DCAT-AP 2.0.1. Diese Eigenschaft kann für parallele Sprachversionen der Beschreibung wiederholt werden.  - `datasetDistribution`: Diese Eigenschaft verknüpft das Dataset mit einer verfügbaren Distributionen. Sie entspricht der obligatorischen Eigenschaft 'Dataset Distribution' von DCAT-AP 2.0.1  - `datasetSource`: Sie entspricht der Eigenschaft 'source' aus DCAT-AP 2.0.1. Diese Eigenschaft verweist auf ein verwandtes Dataset, von dem das beschriebene Dataset abgeleitet ist.  - `datasetType`: Diese Eigenschaft bezieht sich auf den Typ des Datasets. Sie entspricht der Eigenschaft 'Type' des DCAT 2.0.1. Ein kontrolliertes Vokabular für die Werte wurde nicht festgelegt.  - `dateCreated`: Zeitstempel der Entitätserstellung. Dieser wird normalerweise von der Speicherplattform zugewiesen.  - `dateModified`: Zeitstempel der letzten Änderung der Entität. Dieser wird in der Regel von der Speicherplattform vergeben.  - `description`: Eine Beschreibung dieses Artikels  - `documentation`: Diese Eigenschaft verweist auf eine Seite oder ein Dokument zu diesem Dataset  - `frequency`: Diese Eigenschaft bezieht sich auf die Häufigkeit, mit der das Dataset aktualisiert wird.  - `hasVersion`: Diese Eigenschaft bezieht sich auf ein verwandtes Dataset, das eine Version, Ausgabe oder Anpassung des beschriebenen Datasets ist.  - `id`: Eindeutiger Bezeichner der Entität  - `identifier`: Diese Eigenschaft enthält den Hauptbezeichner für das Dataset, z. B. den URI oder einen anderen eindeutigen Bezeichner im Kontext des Katalogs  - `isReferencedBy`: Diese Eigenschaft verweist auf einen verwandten Dataset, von dem der beschriebene Dataset eine Version, Ausgabe oder Anpassung ist  - `isVersionOf`: Diese Eigenschaft enthält den Hauptbezeichner für das Dataset, z. B. den URI oder einen anderen eindeutigen Bezeichner im Kontext des Katalogs  - `keyword`: Diese Eigenschaft enthält ein Schlüsselwort oder Tag, das den Dataset beschreibt  - `landingPage`: Diese Eigenschaft verweist auf eine Webseite, die Zugriff auf den Datensatz, seine Verteilungen und/oder zusätzliche Informationen bietet. Sie soll auf eine Landing Page beim ursprünglichen Datenanbieter verweisen, nicht auf eine Seite auf einer Website eines Dritten, wie z. B. eines Aggregators.  - `language`: Diese Eigenschaft bezieht sich auf eine Sprache des Datasets. Diese Eigenschaft kann wiederholt werden, wenn es mehrere Sprachen im Dataset gibt.  - `name`: Der Name dieses Elements.  - `otherIdentifier`: Diese Eigenschaft bezieht sich auf einen sekundären Identifikator des Datasets, wie MAST/ADS, DataCite, DOI, EZID oder W3ID.  - `owner`: Eine Liste mit einer JSON-kodierten Zeichenfolge, die auf die eindeutigen Ids der Eigentümer verweist  - `provenance`: Diese Eigenschaft enthält eine Aussage über die Abstammung eines Datasets.  - `publisher`: Diese Eigenschaft bezieht sich auf eine Entität (Organisation), die für die Bereitstellung des Datasets verantwortlich ist  - `qualifiedAttribution`: Diese Eigenschaft bezieht sich auf eine verwandte Ressource, z. B. eine Veröffentlichung, die auf den Datensatz verweist, ihn zitiert oder anderweitig auf ihn verweist.  - `qualifiedRelation`: Diese Eigenschaft bezieht sich auf eine verwandte Ressource, z. B. eine Veröffentlichung, die auf den Datensatz verweist, ihn zitiert oder anderweitig auf ihn verweist  - `relatedResource`: Diese Eigenschaft verweist auf eine verwandte Ressource  - `releaseDate`: Diese Eigenschaft enthält das Datum der formalen Herausgabe (z. B. Veröffentlichung) des Datasets.  - `sample`: Diese Eigenschaft bezieht sich auf eine Stichprobenverteilung des Datensatzes  - `seeAlso`: Liste von uri, die auf zusätzliche Ressourcen über das Element verweist  - `source`: Eine Folge von Zeichen, die die ursprüngliche Quelle der Entitätsdaten als URL angibt. Empfohlen wird der voll qualifizierte Domänenname des Quellanbieters oder die URL zum Quellobjekt.  - `spatial`: Diese Eigenschaft bezieht sich auf eine geografische Region, die vom Dataset abgedeckt wird  - `spatialResolution`: Diese Eigenschaft bezieht sich auf den minimalen räumlichen Abstand, der in einem Datensatz auflösbar ist, gemessen in Metern  - `temporal`: Diese Eigenschaft bezieht sich auf einen zeitlichen Bereich, den das Dataset abdeckt  - `temporalResolution`: Diese Eigenschaft bezieht sich auf den minimal auflösbaren Zeitraum im Datensatz.  - `theme`: Diese Eigenschaft bezieht sich auf eine Kategorie des Datasets. Ein Dataset kann mit mehreren Themen verknüpft sein  - `title`: Diese Eigenschaft enthält einen Namen, der dem Dataset gegeben wird. Sie entspricht der Pflichteigenschaft 'Title' von DCAT-AP 2.0.1. Diese Eigenschaft kann für parallele Sprachversionen des Namens wiederholt werden.  - `type`: NGSI-Typ. Es muss Dataset sein  - `updateDate`: Diese Eigenschaft enthält das letzte Datum, an dem das Dataset geändert oder modifiziert wurde.  - `version`: Diese Eigenschaft enthält eine Versionsnummer oder eine andere Versionsbezeichnung des Datasets  - `versionNotes`: Diese Eigenschaft enthält eine Beschreibung der Unterschiede zwischen dieser Version und einer früheren Version des Datasets. Diese Eigenschaft kann für parallele Sprachversionen der Versionshinweise wiederholt werden.  - `wasGeneratedBy`: Diese Eigenschaft enthält eine Beschreibung der Unterschiede zwischen dieser Version und einer früheren Version des Datasets. Diese Eigenschaft kann für parallele Sprachversionen der Versionshinweise wiederholt werden.    
...
#### Dataset NGSI-v2 Schlüsselwerte Beispiel  
Hier ist ein Beispiel für ein Dataset im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-v2 bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
...
Hier ist ein Beispiel für ein Dataset im JSON-LD-Format wie normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden, und liefert die Kontextdaten einer einzelnen Entität.  
...
Hier ist ein Beispiel für ein Dataset im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-LD bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
...
Hier ist ein Beispiel für ein Dataset im JSON-LD-Format wie normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden, und liefert die Kontextdaten einer einzelnen Entität.  
